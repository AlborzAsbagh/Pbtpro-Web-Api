...
namespace WebApiNew.Controllers
{
    ...
    public class IsEmriController : ApiController
    {
		...
		//Get "IsEmri" List for Mobile App Version
...
";
        ...
        //Get "IsEmri" List for Web App Version

        [Route("api/getIsEmriFullList")]
        [HttpGet]
        public List<IsEmriWebVersionModel> GetIsEmriFullList([FromUri] int id)
        {

			#region 
			string query = @"SELECT
                            TOP (10) 
                          ism.TB_ISEMRI_ID AS ISEMRI_ID , 
                          ism.ISM_KAPATILDI AS KAPALI,
                          soc.SOC_TANIM AS ONCELIK,
                          CASE WHEN dsy.DSY_REF_ID IS NOT NULL THEN 1 ELSE 0 END AS BELGE,
                          CASE WHEN rsm.TB_RESIM_ID IS NOT NUll THEN 1 ELSE 0 END AS RESIM,
                          ism.ISM_ISEMRI_NO AS ISEMRI_NO,
                          CASE WHEN mlz.IDM_ISEMRI_ID IS NOT NULL THEN 1 ELSE 0 END AS MALZEME , 
                          CASE WHEN idm.IDK_ISEMRI_ID IS NOT NULL THEN 1 ELSE 0 END AS PERSONEL , 
                          CASE WHEN mkd.MKD_ISEMRI_ID IS NOT NULL THEN 1 ELSE 0 END AS DURUS,
                          ism.ISM_NOT AS OUTER_NOT ,
                          ism.ISM_DUZENLEME_TARIH AS DUZENLEME_TARIH , 
                          ism.ISM_DUZENLEME_SAAT AS DUZENLEME_SAAT,
                          ism.ISM_KONU AS KONU, 
                          /*AS ISEMRI_TIP*/
                          kod_ism_durum.KOD_TANIM AS DURUM ,
                          ...
                          mkn.MKN_PLAKA AS MAKINE_PLAKA ,
                          kod_mkn_durum.KOD_TANIM AS MAKINE_DURUM,
                          kod_mkn_tip.KOD_TANIM AS MAKINE_TIP ,
                          ekp.EKP_TANIM AS EKIPMAN ,
                          ism.ISM_IS_TIP AS IS_TIPI ,
                          kod_is_nedeni.KOD_TANIM AS IS_NDEDNI,
                          atl.ATL_TANIM AS ATOLYE , 
                          tlm.TLM_TANIM AS TALIMAT ,
                          soc.SOC_TANIM AS ONCELIK,
                          ism.ISM_KAPANMA_YDK_TARIH AS KAPANIS_TARIHI , 
                          ism.ISM_KAPANMA_YDK_SAAT AS KAPANIS_SAATI , 
                          tkv.TKV_TANIM AS TAKVIM, 
                          msrf.MAM_TANIM AS MASRAF_MERKEZI,
                          cari.CAR_TANIM AS FRIMA, 
	                        /* IS TALEP NO ?? (Bulunmadi) */
	                        /* IS TALEP EDEN ?? (Bulunmadi) */
	                        /* IS TALEP TARIH ?? (Bulunmadi) */
                          ...
                          kod_ozel_alan_11.KOD_TANIM AS OZEL_ALAN_11 ,
                          kod_ozel_alan_12.KOD_TANIM AS OZEL_ALAN_12 ,
                          kod_ozel_alan_13.KOD_TANIM AS OZEL_ALAN_13 ,
                          kod_ozel_alan_14.KOD_TANIM AS OZEL_ALAN_14 ,
                          kod_ozel_alan_15.KOD_TANIM AS OZEL_ALAN_15 ,
                          ...
                          /* Bildirlen Kat 
	                        Bildirlen Bina ?? (Bulunmadi) */
                          prs.PRS_ISIM AS PERSONEL_ADI ,
                          lok.LOK_TUM_YOL AS TAM_LOKASYON ,
                          ism.ISM_SAYAC_DEGER AS GUNCEL_SAYAC_DEGER ,
                          ism.ISM_MAKINE_GUVENLIK_NOTU AS ICERDEKI_NOT

                        FROM
                          orjin.TB_ISEMRI ism
                        LEFT JOIN orjin.TB_SERVIS_ONCELIK soc ON ism.ISM_ONCELIK_ID = soc.TB_SERVIS_ONCELIK_ID 
                        LEFT JOIN dbo.TB_DOSYA dsy ON ism.TB_ISEMRI_ID = dsy.DSY_REF_ID AND dsy.DSY_REF_GRUP = 'ISEMRI' 
                        LEFT JOIN orjin.TB_RESIM rsm ON ism.TB_ISEMRI_ID = rsm.TB_RESIM_ID AND rsm.RSM_REF_GRUP = 'ISEMRI' 
                        LEFT JOIN orjin.TB_ISEMRI_MLZ mlz ON ism.TB_ISEMRI_ID = mlz.IDM_ISEMRI_ID 
                        LEFT JOIN orjin.TB_ISEMRI_KAYNAK idm ON ism.TB_ISEMRI_ID = idm.IDK_ISEMRI_ID 
                        LEFT JOIN orjin.TB_MAKINE_DURUS mkd ON ism.TB_ISEMRI_ID = mkd.MKD_ISEMRI_ID 
                        /* LEFT JOIN orjin.TB_ISEMRI_TIP imt ON ism.TB_ISEMRI_ID = imt.ISM_TIP_ID */
                        LEFT JOIN orjin.TB_KOD kod_ism_durum ON ism.ISM_DURUM_KOD_ID = kod_ism_durum.TB_KOD_ID 
                        ...
                        LEFT JOIN orjin.TB_KOD kod_mkn_tip ON mkn.MKN_TIP_KOD_ID = kod_mkn_tip.TB_KOD_ID 
                        LEFT JOIN orjin.TB_KOD kod_mkn_durum ON mkn.MKN_DURUM_KOD_ID = kod_mkn_durum.TB_KOD_ID 
                        ...
                        LEFT JOIN orjin.TB_MASRAF_MERKEZ msrf ON ism.ISM_MASRAF_MERKEZ_ID = msrf.TB_MASRAF_MERKEZ_ID  
                        LEFT JOIN orjin.TB_CARI cari ON ism.ISM_FIRMA_ID = cari.TB_CARI_ID
                        LEFT JOIN orjin.TB_KOD kod_ozel_alan_11 ON ism.ISM_OZEL_ALAN_11_KOD_ID = kod_ozel_alan_11.TB_KOD_ID
                        LEFT JOIN orjin.TB_KOD kod_ozel_alan_12 ON ism.ISM_OZEL_ALAN_12_KOD_ID = kod_ozel_alan_12.TB_KOD_ID
                        LEFT JOIN orjin.TB_KOD kod_ozel_alan_13 ON ism.ISM_OZEL_ALAN_13_KOD_ID = kod_ozel_alan_13.TB_KOD_ID
                        LEFT JOIN orjin.TB_KOD kod_ozel_alan_14 ON ism.ISM_OZEL_ALAN_14_KOD_ID = kod_ozel_alan_14.TB_KOD_ID
                        LEFT JOIN orjin.TB_KOD kod_ozel_alan_15 ON ism.ISM_OZEL_ALAN_15_KOD_ID = kod_ozel_alan_15.TB_KOD_ID
                        LEFT JOIN orjin.TB_PERSONEL prs ON idm.IDK_REF_ID = prs.TB_PERSONEL_ID ";
			#endregion

			List<IsEmriWebVersionModel> listem = new List<IsEmriWebVersionModel>();
            ...
            //{
				...
				{
					IsEmriWebVersionModel entity = new IsEmriWebVersionModel();
					entity.TB_ISEMRI_ID = (int)dt.Rows[i]["ISEMRI_ID"];
					entity.KAPALI = dt.Rows[i]["KAPALI"] != DBNull.Value ? (bool)dt.Rows[i]["KAPALI"] : false;
					entity.ONCELIK = dt.Rows[i]["ONCELIK"] != DBNull.Value ? (string)dt.Rows[i]["ONCELIK"] : ""; ;
					entity.BELGE = dt.Rows[i]["BELGE"] != DBNull.Value ? (int)dt.Rows[i]["BELGE"] : 0;
					entity.RESIM = dt.Rows[i]["RESIM"] != DBNull.Value ? (int)dt.Rows[i]["RESIM"] : 0;
					entity.ISEMRI_NO = dt.Rows[i]["ISEMRI_NO"] != DBNull.Value ? (string)dt.Rows[i]["ISEMRI_NO"] : "";
					entity.MALZEME = dt.Rows[i]["MALZEME"] != DBNull.Value ? (int)dt.Rows[i]["MALZEME"] : 0;
					entity.PERSONEL = dt.Rows[i]["PERSONEL"] != DBNull.Value ? (int)dt.Rows[i]["PERSONEL"] : 0;
					entity.DURUS = dt.Rows[i]["DURUS"] != DBNull.Value ? (int)dt.Rows[i]["DURUS"] : 0;
					entity.OUTER_NOT = dt.Rows[i]["OUTER_NOT"] != DBNull.Value ? (string)dt.Rows[i]["OUTER_NOT"] : "";
					entity.DUZENLEME_TARIH = dt.Rows[i]["DUZENLEME_TARIH"] != DBNull.Value ? (DateTime)dt.Rows[i]["DUZENLEME_TARIH"] :DateTime.MinValue ;
					entity.DUZENLEME_SAAT = dt.Rows[i]["DUZENLEME_SAAT"] != DBNull.Value ? (string)dt.Rows[i]["DUZENLEME_SAAT"] : "";
					entity.KONU = dt.Rows[i]["KONU"] != DBNull.Value ? (string)dt.Rows[i]["KONU"] : "";

					entity.DURUM = dt.Rows[i]["DURUM"] != DBNull.Value ? (string)dt.Rows[i]["DURUM"] : "";
					entity.LOKASYON = dt.Rows[i]["LOKASYON"] != DBNull.Value ? (string)dt.Rows[i]["LOKASYON"] : "";
					entity.PLAN_BASLAMA_TARIH = dt.Rows[i]["PLAN_BASLAMA_TARIH"] != DBNull.Value ? (DateTime)dt.Rows[i]["PLAN_BASLAMA_TARIH"]:DateTime.MinValue ;
					entity.PLAN_BASLAMA_SAAT = dt.Rows[i]["PLAN_BASLAMA_SAAT"] != DBNull.Value ? (string)dt.Rows[i]["PLAN_BASLAMA_SAAT"] : "";
					entity.PLAN_BITIS_TARIH = dt.Rows[i]["PLAN_BITIS_TARIH"] != DBNull.Value ? (DateTime)dt.Rows[i]["PLAN_BITIS_TARIH"] : DateTime.MinValue;
					entity.PLAN_BITIS_SAAT = dt.Rows[i]["PLAN_BITIS_SAAT"] != DBNull.Value ? (string)dt.Rows[i]["PLAN_BITIS_SAAT"] : "";
					entity.BASLAMA_TARIH = dt.Rows[i]["BASLAMA_TARIH"] != DBNull.Value ? (DateTime)dt.Rows[i]["BASLAMA_TARIH"] : DateTime.MinValue;
					entity.BASLAMA_SAAT = dt.Rows[i]["BASLAMA_SAAT"] != DBNull.Value ? (string)dt.Rows[i]["BASLAMA_SAAT"] : "";
					...
					entity.IS_SURESI = dt.Rows[i]["IS_SURESI"] != DBNull.Value ? (int)dt.Rows[i]["IS_SURESI"] : 0;
					entity.TAMAMLANMA = dt.Rows[i]["TAMAMLANMA"] != DBNull.Value ? (int)dt.Rows[i]["TAMAMLANMA"] : 0;
					entity.GARANTI = dt.Rows[i]["GARANTI"] != DBNull.Value ? (bool)dt.Rows[i]["GARANTI"] : false ;
					entity.MAKINE_KODU = dt.Rows[i]["MAKINE_KODU"] != DBNull.Value ? (string)dt.Rows[i]["MAKINE_KODU"] : "";
					entity.MAKINE_TANIMI = dt.Rows[i]["MAKINE_TANIMI"] != DBNull.Value ? (string)dt.Rows[i]["MAKINE_TANIMI"] : "";
					entity.MAKINE_PLAKA = dt.Rows[i]["MAKINE_PLAKA"] != DBNull.Value ? (string)dt.Rows[i]["MAKINE_PLAKA"] : "";
					entity.MAKINE_DURUM = dt.Rows[i]["MAKINE_DURUM"] != DBNull.Value ? (string)dt.Rows[i]["MAKINE_DURUM"] : "";
					entity.MAKINE_TIP = dt.Rows[i]["MAKINE_TIP"] != DBNull.Value ? (string)dt.Rows[i]["MAKINE_TIP"] : "";
					entity.EKIPMAN = dt.Rows[i]["EKIPMAN"] != DBNull.Value ? (string)dt.Rows[i]["EKIPMAN"] : "";
					entity.IS_TIPI = dt.Rows[i]["IS_TIPI"] != DBNull.Value ? (string)dt.Rows[i]["IS_TIPI"] : "";
					entity.IS_NDEDNI = dt.Rows[i]["IS_NDEDNI"] != DBNull.Value ? (string)dt.Rows[i]["IS_NDEDNI"] : "";
					entity.ATOLYE = dt.Rows[i]["ATOLYE"] != DBNull.Value ? (string)dt.Rows[i]["ATOLYE"] : "";
					entity.TALIMAT = dt.Rows[i]["TALIMAT"] != DBNull.Value ? (string)dt.Rows[i]["TALIMAT"] : "";
					entity.KAPANIS_TARIHI = dt.Rows[i]["KAPANIS_TARIHI"] != DBNull.Value ? (DateTime)dt.Rows[i]["KAPANIS_TARIHI"] : DateTime.MinValue;
					entity.KAPANIS_SAATI = dt.Rows[i]["KAPANIS_SAATI"] != DBNull.Value ? (string)dt.Rows[i]["KAPANIS_SAATI"] : "";
					entity.TAKVIM = dt.Rows[i]["TAKVIM"] != DBNull.Value ? (string)dt.Rows[i]["TAKVIM"] : "";
					entity.MASRAF_MERKEZI = dt.Rows[i]["MASRAF_MERKEZI"] != DBNull.Value ? (string)dt.Rows[i]["MASRAF_MERKEZI"] : "";
					entity.FRIMA = dt.Rows[i]["FRIMA"] != DBNull.Value ? (string)dt.Rows[i]["FRIMA"] : "";

					entity.OZEL_ALAN_1 = dt.Rows[i]["OZEL_ALAN_1"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_1"] : "";
					entity.OZEL_ALAN_2 = dt.Rows[i]["OZEL_ALAN_2"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_2"] : "";
					entity.OZEL_ALAN_3 = dt.Rows[i]["OZEL_ALAN_3"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_3"] : "";
					entity.OZEL_ALAN_4 = dt.Rows[i]["OZEL_ALAN_4"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_4"] : "";
					entity.OZEL_ALAN_5 = dt.Rows[i]["OZEL_ALAN_5"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_5"] : "";
					entity.OZEL_ALAN_6 = dt.Rows[i]["OZEL_ALAN_6"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_6"] : "";
					entity.OZEL_ALAN_7 = dt.Rows[i]["OZEL_ALAN_7"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_7"] : "";
					entity.OZEL_ALAN_8 = dt.Rows[i]["OZEL_ALAN_8"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_8"] : "";
					entity.OZEL_ALAN_9 = dt.Rows[i]["OZEL_ALAN_9"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_9"] : "";
					entity.OZEL_ALAN_10 = dt.Rows[i]["OZEL_ALAN_10"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_10"] : "";
					entity.OZEL_ALAN_11 = dt.Rows[i]["OZEL_ALAN_11"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_11"] : "";
					entity.OZEL_ALAN_12 = dt.Rows[i]["OZEL_ALAN_12"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_12"] : "";
					entity.OZEL_ALAN_13 = dt.Rows[i]["OZEL_ALAN_13"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_13"] : "";
					entity.OZEL_ALAN_14 = dt.Rows[i]["OZEL_ALAN_14"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_14"] : "";
					entity.OZEL_ALAN_15 = dt.Rows[i]["OZEL_ALAN_15"] != DBNull.Value ? (string)dt.Rows[i]["OZEL_ALAN_15"] : "";
					entity.OZEL_ALAN_16 = dt.Rows[i]["OZEL_ALAN_16"] != DBNull.Value ? (double)dt.Rows[i]["OZEL_ALAN_16"] : 0;
					entity.OZEL_ALAN_17 = dt.Rows[i]["OZEL_ALAN_17"] != DBNull.Value ? (double)dt.Rows[i]["OZEL_ALAN_17"] : 0;
					entity.OZEL_ALAN_18 = dt.Rows[i]["OZEL_ALAN_18"] != DBNull.Value ? (double)dt.Rows[i]["OZEL_ALAN_18"] : 0;
					entity.OZEL_ALAN_19 = dt.Rows[i]["OZEL_ALAN_19"] != DBNull.Value ? (double)dt.Rows[i]["OZEL_ALAN_19"] : 0;
					entity.OZEL_ALAN_20 = dt.Rows[i]["OZEL_ALAN_20"] != DBNull.Value ? (double)dt.Rows[i]["OZEL_ALAN_20"] : 0;

					entity.PERSONEL_ADI = dt.Rows[i]["PERSONEL_ADI"] != DBNull.Value ? (string)dt.Rows[i]["PERSONEL_ADI"] : "";
					entity.TAM_LOKASYON = dt.Rows[i]["TAM_LOKASYON"] != DBNull.Value ? (string)dt.Rows[i]["TAM_LOKASYON"] : "";
					entity.GUNCEL_SAYAC_DEGER = dt.Rows[i]["GUNCEL_SAYAC_DEGER"] != DBNull.Value ? (int)dt.Rows[i]["GUNCEL_SAYAC_DEGER"] : 0;
					entity.ICERDEKI_NOT = dt.Rows[i]["ICERDEKI_NOT"] != DBNull.Value ? (string)dt.Rows[i]["ICERDEKI_NOT"] : "";
